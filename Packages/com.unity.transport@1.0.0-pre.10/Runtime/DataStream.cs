...
namespace Unity.Networking.Transport
{
    ...
    public unsafe struct DataStreamWriter : IDisposable
    {
        ...
        public DataStreamWriter(int length, Allocator allocator)
        {
            ...
            Initialize(out this, new NativeArray<byte>(length, allocator), allocator);
        }
        ...
        private static void Initialize(out DataStreamWriter self, NativeArray<byte> data, Allocator allocator = Allocator.Temp)
        {
        ...
        public void Dispose()
        {
            if (m_allocator == Allocator.TempJob)
            {
                var na = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<byte>(m_Data.buffer, Length, Allocator.TempJob);
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                NativeArrayUnsafeUtility.SetAtomicSafetyHandle(ref na, m_Safety);
#endif
                na.Dispose();
    ...
    public unsafe struct DataStreamReader
    {
        ...
        {
            ReadBytes((byte*)array.GetUnsafeReadOnlyPtr(), array.Length);
        }
